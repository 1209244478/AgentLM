...
- 支持 **In-Place TTT（推理时训练）**：推理时在 FFN down_proj 上做 mini-batch SGD，无需额外参数即可实现上下文内化，生成接口原生支持 `use_ttt=True`。
- 支持 **跨层参数共享（Layer Sharing）**：通过 `layer_share_factor` 控制每 N 层共享一组参数，显著压缩模型体积，与 MoE / TTT / MTP 全兼容。
...
- 🧪 **离散扩散语言模型**（[dLM](https://github.com/jingyaogong/minimind/discussions/618)）与线性注意力模型（[Linear Attention](https://github.com/jingyaogong/minimind/discussions/704)），均可基于主线 AR 模型进行续训。

#### 🎉 已发布模型列表

| 模型 | 参数量 | Release |
|------|--------|---------|
| minimind-5 | 186M | 2026.05 |
...
- **MiniMind-5 成为默认训练配置**：`hidden_size=1024, num_hidden_layers=12`，参数量 ~186M（含 TTT + MTP 头），训练脚本默认使用 MiniMind-5 配置。
- **数据集扩展**：新增 MINT-1T ArXiv 学术论文数据（30K pretrain + 10K SFT），与原始数据合并为 `pretrain_combined.jsonl`（130 万条）和 `sft_combined.jsonl`（91 万条），训练脚本默认指向合并数据。
- **Transformer 性能优化**：
  - Attention 三级路由：Flash Attention 2（训练）→ PyTorch SDPA（推理+KV Cache）→ Manual Attention（Fallback）
  - KVCache 预分配：推理时固定大小 buffer + `copy_` 原地写入，避免 `torch.cat` 的 O(n²) 显存分配
  - SDPA + KV Cache 兼容：推理时正确构造 `attn_mask`，走 SDPA 快速路径
  - RoPE 优化：直接拆分计算，减少中间张量分配
  - RMSNorm 内联：消除独立 `norm()` 方法，`torch.compile` 友好
  - Generate Prefill：先一次处理完整 prompt，再逐 token decode，首 token 延迟降低 10x+
- **数据转换脚本**：新增 `dataset/convert_mint_arxiv.py`，支持从 HuggingFace MINT-1T 下载、LaTeX 清洗、格式转换，支持 SSL 超时重试和断点续传。

</details>

<details> 
<summary> <b>🔥 2026-05 (MiniMind-5 架构更新)</b> </summary>

- **In-Place TTT（推理时训练）**：在 FFN down_proj 层实现推理时 mini-batch SGD 权重更新，模型可在前向传播时根据当前上下文自调整参数，推理完成后通过 `disable_ttt()` 恢复原始权重。`generate()` 原生支持 `use_ttt=True`。
- **跨层参数共享**：通过 `layer_share_factor` 参数控制每 N 层共享一组参数，显著压缩模型体积，与 TTT / MoE / MTP 全兼容。
...
- **统一测试**：新增 `tests/test_all.py`，27 项测试覆盖模型构建、TTT、MTP、MHC+CSA、Muon、OPD、生成等全流程。
- **项目结构精简**：合并冗余模块、移除实验文件、统一测试入口，回归"大道至简"的初衷。
...
> 训练后，将得到 `out/pretrain_*.pth` 作为输出权重（其中 `*` 为模型 dimension，默认为 `1024`）
>
> 默认使用 MiniMind-5 配置（1024, 12L, ~186M），数据为 `pretrain_combined.jsonl`
>
> 如需使用 MiniMind-3 旧配置：`--hidden_size 768 --num_hidden_layers 8 --data_path ../dataset/pretrain_t2t_mini.jsonl`

#### 2.2 指令微调（必须）

```bash
cd trainer && python train_full_sft.py --from_weight pretrain
```

> 训练后，将得到 `out/full_sft_*.pth` 作为输出权重（其中 `full` 表示全参数微调）
>
> 默认使用 MiniMind-5 配置，数据为 `sft_combined.jsonl`

<details>
<summary>可选：使用 Muon 优化器（节省显存，提升收敛）</summary>

```bash
cd trainer && python train_full_sft.py --optimizer muon --learning_rate 1e-3
```

Muon 基于动量梯度 + Newton-Schulz 正交归一化，对 Linear 层自动使用 Muon 更新，Embedding/Norm 层回退为 AdamW，可节省约 50% 优化器状态显存。

切换回默认 AdamW：去掉 `--optimizer muon` 参数即可。

| 关键参数 | 默认值 | 说明 |
|----------|--------|------|
| `--optimizer` | adamw | 优化器：`adamw` / `muon` |
| `--hidden_size` | 768 | 隐藏层维度 |
| `--num_hidden_layers` | 8 | Transformer 层数 |
| `--max_seq_len` | 768 | 训练截断长度 |
| `--use_moe` | 0 | MoE 架构开关 |
| `--from_weight` | pretrain | 基于哪个权重热启 |
| `--epochs` | 2 | 训练轮数 |
| `--learning_rate` | 1e-5 | 学习率（Muon 推荐 1e-3） |
...
1、所有训练脚本均基于 PyTorch 原生实现，并支持多卡加速。训练脚本均支持 `--help` 查看完整参数列表。
...
`trainer/trainer_utils.py` 中内置 Muon 优化器（DeepSeek V4 风格）：
- **MixedMuonAdamW**：≥2D 大矩阵用 Newton-Schulz 正交归一化（Muon），1D/小矩阵用 AdamW
- 通过 `--optimizer muon` 参数即可切换，节省约 50% 优化器状态显存

修改模型配置见[./model/model_minimind.py](./model/model_minimind.py)，高级架构模块见 [./model/model_advanced.py](./model/model_advanced.py)（mHC + CSA），全模态模型见 [./model/model_omni.py](./model/model_omni.py)。参考模型参数版本见下表：

| Model Name | params | len_vocab | max_pos | rope_theta | n_layers | d_model | kv_heads | q_heads | note |
|------------|--------|-----------|---------|------------|----------|---------|----------|---------|------|
| minimind-5 | 104M | 32000 | 32768 | 1e6 | 12 | 1024 | 8 | 16 | TTT+MTP+LayerShare |
...
以下特性深度集成在核心模型代码中，通过 `MiniMindConfig` 参数即可启用，无需额外代码：

#### In-Place TTT（推理时训练）

TTT 在推理时对 FFN 的 `down_proj` 层做 mini-batch SGD：按 `ttt_chunk_size` 分块前向，每块结束时用自身 hidden states 计算重建损失作为梯度信号去更新权重。整个过程复用 FFN 的现有结构，不引入额外参数，推理结束后通过 `disable_ttt()` 即可恢复原始权重。

```python
config = MiniMindConfig(hidden_size=1024, num_hidden_layers=12,
                        ttt_enabled=True, ttt_chunk_size=512, ttt_lr=1e-4)
model = MiniMindForCausalLM(config)
model.enable_ttt()                          # 启用 TTT
output = model(input_ids, use_ttt=True)     # 推理时自动更新权重
model.disable_ttt()                         # 恢复原始权重

# generate() 原生支持
model.generate(input_ids, use_ttt=True, ttt_interval=64)
```

默认仅在最后 25% 层启用 TTT（可通过 `ttt_layers` 自定义）。相关代码见 `model/model_minimind.py` 中的 `TTTFeedForward` 类。

#### 跨层参数共享

通过 `layer_share_factor` 控制每 N 层共享一组参数：

```python
config = MiniMindConfig(hidden_size=1024, num_hidden_layers=12, layer_share_factor=2)
# 12 层只需 6 组 unique blocks (0→0, 1→1, 2→0, 3→1, ...)
```

| layer_share_factor | 12 层 unique blocks | 参数量（相对） |
|---|---|---|
| 1（不共享） | 12 | 100% |
| 2 | 6 | ~55% |
| 3 | 4 | ~40% |

与 TTT、MoE、MTP 完全兼容，`enable_ttt()` / `forward()` 自动适配。

#### 多 Token 预测（MTP）

在 lm_head 后追加 N 个 `MTPHead`，每个预测头学习未来 token 分布，MTP loss 作为辅助损失自动加权到总 loss：

```python
config = MiniMindConfig(mtp_num_heads=2, mtp_loss_weight=0.1)
# loss = text_loss + 0.1 * (mtp_head_1_loss + mtp_head_2_loss)
```

前向传播中自动计算并合并 MTP 损失，无需额外训练代码。

#### Muon 优化器

基于 DeepSeek V4 的动量矩优化器，通过 Newton-Schulz 迭代将梯度矩阵投影到正交矩阵，保留方向信息、消除尺度偏差。混合策略：≥2D 大矩阵用 Muon，1D/小矩阵回退 AdamW。见 `trainer/trainer_utils.py`。
...
TTT 的 `predictor` 从随机初始化开始，更新方向不一定最优。Meta-TTT 用 FOMAML 一阶近似在外循环训练 predictor：内循环做 TTT 更新，外循环在 meta-task 上计算 loss 并反传回 predictor，让模型学会"如何在推理时自更新"。

```bash
cd trainer && python train_meta_ttt.py
```

#### 🧪 诚实训练与置信度校准

传统 RL reward 函数容易被 hack。诚实训练通过三个维度评分：
1. **置信度校准**：用生成序列的平均熵值估算模型信心
2. **诚实放弃检测**：识别模型是否主动表达"不确定"/"不会"
3. **GT 正确性验证**：鲁棒匹配回答与 ground truth

分层奖励：正确+自信(2.0) > 正确+犹豫(1.0) > 诚实放弃(0.3) > 错误+犹豫(-1.0) > 错误+自信(-2.0)

已集成到 `train_grpo.py` / `train_ppo.py` / `train_agent.py` 的奖励计算中，核心逻辑见 `trainer/honest_training.py`。

---

## 🧪 全模态 Omni 模型

MiniMind Omni 基于 [MiniMind-O](https://github.com/jingyaogong/minimind-o) 的 Thinker-Talker 双路径架构实现，在 `MiniMindForCausalLM` 基础上扩展为全模态模型。
...
输入: 文本 + 音频波形 + 图像
           │
     ┌─────▼──────┐     ┌──────────────┐
     │ SenseVoice  │     │  SigLIP-SO   │
     │ (frozen)    │     │  (frozen)    │
     └─────┬──────┘     └──────┬───────┘
           │                   │
     ┌─────▼──────┐     ┌──────▼───────┐
     │AudioProjector│    │VisionProjector│   ← 可训练 MLP
     └─────┬──────┘     └──────┬───────┘
           │                   │
           └───────┬───────────┘
                   │
         ┌─────────▼─────────┐
         │     Thinker        │   ← 复用 MiniMindModel
         │  (layers 0..N-1)   │      文本理解 + 多模态融合
         │   ──Bridge──▶      │      bridge_layer 处截取 hidden states
         └────────┬──────────┘
                  │ bridge_states
         ┌────────▼──────────┐
         │     Talker         │   ← 独立 4 层 MiniMindBlock
         │   embed_proj +     │      embed_proj * text_scale +
         │   codec_proj       │      codec_proj * audio_scale
         ├────────────────────┤
         │    TalkerHead      │   ← MTP 风格 8 层音频头
         │  (base + 8 adapter)│      预测 8 层 Mimi codes
         └────────┬──────────┘
                  │
        ┌─────────▼─────────┐
        │  Text + Audio 输出  │
...
| 组件 | 文件 | 说明 |
|------|------|------|
| `OmniConfig` | `model/model_omni.py` | 扩展 `MiniMindConfig`，新增 `bridge_layer` / `audio_ids` / `image_ids` / `talker_hidden_size` 等 |
| `MiniMindOmni` | `model/model_omni.py` | 主模型，继承 `MiniMindForCausalLM`，实现 Thinker-Talker 双路径 forward |
| `MMAudioProjector` | `model/model_omni.py` | SenseVoice(512) → Thinker(768) 特征映射 |
| `MMVisionProjector` | `model/model_omni.py` | SigLIP(768) → Thinker(768) 特征映射 |
| `TalkerHead` | `model/model_omni.py` | MTP 风格：base + 8 个 low-rank adapter（rank=256）预测 8 层 Mimi codes |
| `TalkerEmbedding` | `model/model_omni.py` | 8 层 codebook 的 embedding，支持 audio code 序列化 |
| `TalkerModule` | `model/model_omni.py` | 独立 Talker 层（4 层 MiniMindBlock），接收 bridge_states 生成音频 |
| `OmniDataset` | `dataset/omni_dataset.py` | 全模态数据集，支持文本/语音/图像三模态，数据增强、scheduled sampling |
| `train_sft_omni` | `trainer/train_sft_omni.py` | Omni SFT 训练入口，Text Loss + Audio Loss 联合训练，支持从 Thinker 权重热启 Talker |
| `SileroVAD` / `RealtimeSession` | `model/model_omni.py` | 实时 VAD 打断 + 流式输入/输出对话会话 |
...
# 非流式生成（文本 + 音频码）
text_ids, audio_codes = model.generate(
    input_ids, max_new_tokens=512, stream=False,
    use_cache=True, return_audio_codes=True
)

# 流式生成（实时 token 级别）
for text_token, audio_frame in model.stream_generate(...):
    # text_token: 当前生成的文本 token
    # audio_frame: 当帧音频码（8 层 Mimi codes 对齐后产出）
    ...
```
...
```bash
# 统一测试（27 项）：模型构建、TTT、MTP、MHC+CSA、Muon、OPD、生成
python tests/test_all.py
```

覆盖内容：模型构建、前向传播（文本/音频/图像）、Bridge 层隔离、Talker 梯度流、TTT 集成、跨层共享、generate() 端到端、Speaker Embedding 注入。

> 注：SenseVoice、SigLIP、Mimi 编码器需要安装对应依赖并下载模型权重才能完整启用音频/视觉模态。当外部编码器不可用时，Omni 模型会优雅降级为纯文本模式。
...
* 🧪 [model/model_omni.py](./model/model_omni.py) — 全模态 Omni 模型（Thinker-Talker 双路径架构）

* 🧪 [trainer/honest_training.py](./trainer/honest_training.py) — 诚实训练与置信度校准
...
- [Learning to (Learn at Test Time): RNNs with Expressive Hidden States](https://arxiv.org/abs/2407.04620)（TTT 核心思路来源）
- [Model-Agnostic Meta-Learning for Fast Adaptation of Deep Networks](https://arxiv.org/abs/1703.03400)（MAML / FOMAML 参考）
