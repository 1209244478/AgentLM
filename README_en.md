...
- **MiniMind-5 is now the default training configuration**: `hidden_size=1024, num_hidden_layers=12`, ~186M parameters (including TTT + MTP heads), training scripts default to MiniMind-5 configuration.
- **Dataset expansion**: Added MINT-1T ArXiv academic paper data (30K pretrain + 10K SFT), merged with original data into `pretrain_combined.jsonl` (1.3M entries) and `sft_combined.jsonl` (916K entries), training scripts default to combined datasets.
- **Transformer performance optimizations**:
  - Attention three-level routing: Flash Attention 2 (training) → PyTorch SDPA (inference + KV Cache) → Manual Attention (Fallback)
  - KVCache pre-allocation: fixed-size buffer + `copy_` in-place writes during inference, avoiding O(n²) memory allocation from `torch.cat`
  - SDPA + KV Cache compatibility: correctly construct `attn_mask` during inference to use SDPA fast path
  - RoPE optimization: direct split computation `q1*cos - q2*sin`, reducing intermediate tensor allocation
  - RMSNorm inlining: eliminate standalone `norm()` method, `torch.compile` friendly
  - Generate Prefill: process full prompt at once, then decode token by token, first-token latency reduced 10x+
- **Data conversion script**: Added `dataset/convert_mint_arxiv.py`, supporting HuggingFace MINT-1T download, LaTeX cleaning, format conversion, with SSL timeout retry and checkpoint resume.
...
> After training, `out/pretrain_*.pth` will be produced as output weights (where `*` is the model dimension, default `1024`)
>
> Default uses MiniMind-5 configuration (1024, 12L, ~186M), data is `pretrain_combined.jsonl`
>
> To use the older MiniMind-3 configuration: `--hidden_size 768 --num_hidden_layers 8 --data_path ../dataset/pretrain_t2t_mini.jsonl`
...
>
> Default uses MiniMind-5 configuration, data is `sft_combined.jsonl`
...
|------------|--------|-----------|---------|------------|----------|---------|----------|---------|------|
| minimind-5 | 186M | 6400 | 32768 | 1e6 | 12 | 1024 | 8 | 16 | Dense + TTT + MTP (Default) |
