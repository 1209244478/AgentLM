...
def calculate_rewards(prompts, completions, reward_model, entropies=None):
    """诚实性奖励计算

    移除了可被 hack 的启发式指标，仅使用 RM 打分 + 诚实性检测 + 重复惩罚。
    支持熵值辅助置信度估计。
    """
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            for j in range(args.num_generations):
                ...
                response = completions[response_idx]
                prompt = prompts[i]
                ...
                if '</think>' in response:
                    _, answer = response.split('</think>', 1)
                    answer = answer.strip()

                entropy = entropies[response_idx] if entropies else None
                is_withdrawal, confidence = detect_honest_withdrawal(answer, entropy=entropy)
        ...
        rewards[:] = reward_model_scores.clamp(-3.0, 3.0)
